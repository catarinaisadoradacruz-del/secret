                  ...
                  >
                    ...
                    {editingSession !== s.id && (
                      <div className="absolute right-1.5 top-1/2 -translate-y-1/2">
                        <button onClick={(e) => { e.stopPropagation(); setMenuOpen(menuOpen === s.id ? null : s.id); }} className="p-1.5 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-gray-200 transition-all">
                          <MoreVertical className="w-4 h-4 text-gray-500" />
                        </button>
                        {menuOpen === s.id && (
                          <div className="absolute right-0 top-8 bg-white border rounded-lg shadow-lg py-1 z-20 min-w-[120px]">
                            <button onClick={(e) => startEditSession(s, e)} className="w-full px-3 py-2 text-left text-sm hover:bg-gray-100 flex items-center gap-2"><Edit2 className="w-4 h-4" /> Renomear</button>
                            <button onClick={(e) => deleteSession(s.id, e)} className="w-full px-3 py-2 text-left text-sm hover:bg-red-50 text-red-600 flex items-center gap-2"><Trash2 className="w-4 h-4" /> Excluir</button>
                          </div>
                        )}
                      </div>
                    )}
      ...
      {showMobileSidebar && (
        <>
          ...
          <div className="fixed inset-y-0 left-0 z-50 w-72 bg-white flex flex-col md:hidden">
            ...
            <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-1">
              {sessions.map(s => (
                <button key={s.id} onClick={() => loadMessages(s.id)} className={`w-full text-left p-2.5 rounded-xl transition-colors ${currentSession === s.id ? 'bg-primary-50' : 'hover:bg-gray-100'}`}>
                  <div className="flex items-center gap-2">
